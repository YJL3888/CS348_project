...
def get_menu(restaurant_id):
    with create_connection() as conn, conn.cursor() as cursor:
        cursor.execute("SELECT item_name, price FROM Items WHERE restaurant_id = %s", (restaurant_id,))
...
def get_restaurant(restaurant_id):
    with create_connection() as conn, conn.cursor(prepared=True, dictionary=True) as cursor:
...
def get_favorites():
    with create_connection() as conn, conn.cursor() as cursor:
        cursor.execute("SELECT restaurant_id FROM Favorites WHERE user_id = %s", (current_user['user_id'],))
        rows = cursor.fetchall()
